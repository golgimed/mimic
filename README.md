...
src/
    core/
        server.ts      # bootstraps Fastify, shared infra, registers enabled providers
        registry.ts     # provider registry (registerProvider / getEnabledProviders)
    ...
    server.ts            # CLI entrypoint (calls core/server.ts, starts listening)
```

Each provider owns its routes, request/response payloads, state transitions, and webhook events. The core only starts Fastify, loads configuration, registers enabled providers, and exposes shared infrastructure (logging, scheduling, storage, webhook dispatcher, fault injection). Shared modules under `shared/` only exist because they're reused by more than one provider — nothing there knows a provider's business rules.

### Provider registration

A provider is any module exporting a `Provider` (see `src/core/registry.ts`):

```ts
export interface Provider {
  name: string; // also used as the route prefix, e.g. "zenvia" -> /zenvia/...
  register(app: FastifyInstance): void | Promise<void>;
  listItems?(): DashboardItem[];        // optional: feeds the dashboard
  getItemDetail?(id: string): ItemDetail | undefined;
}
```

`src/providers/index.ts` registers every known provider into the registry. `core/server.ts` mounts only the *enabled* ones (see Configuration below) under `/<provider.name>`.

### Adding a new provider

1. Create `src/providers/<name>/` with routes, handlers, state, and a `README.md` documenting its endpoints and known limitations.
2. Add a `provider.ts` exporting a `Provider` object (see above).
...
- **[Zenvia](src/providers/zenvia/README.md)** — SMS, WhatsApp, and Email messaging, with subscription-based delivery-status webhooks.
- **[IntegraICP](src/providers/integraicp/README.md)** — Brazilian digital signature / ICP-Brasil certificate flows.
...
```bash
MIMIC_PROVIDERS=zenvia npm run dev
```
